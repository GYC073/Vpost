...
**Last updated:** 2026-06-12 (session 30) — **Verify bài auto 12/06: giọng KHÁ HƠN nhưng còn 2 nhóm lỗi.** (1) Vẫn bịa từ ("chợp cơ hội", "để nặng đổ lên", câu gãy) → gốc rễ là Haiku yếu tiếng Việt, Ben chốt **nâng model auto-generate-post lên `claude-sonnet-4-6`** (~200đ/bài, 1 bài/ngày). (2) **Bịa flash sale + bịa chuyện khách** → thủ phạm là TOPIC_POOL có sẵn topic "flash sale hoặc ưu đãi sắp tới" + "câu chuyện khách hàng thật" → ĐÃ BỎ 2 topic này; thêm rule prompt: cấm bịa sự kiện/chuyện khách + xưng hô nhất quán (bài 12/06 nhảy "mình"→"E"). Lưu ý: Edit tool truncate cả index.ts LẪN PROGRESS.md → đều khôi phục từ HEAD + sửa bằng Python. `generate-caption` VẪN dùng Haiku (user-facing, nhiều call — chưa nâng). ⏳ Ben: xoá `.git\index.lock` từ Windows → commit + push → deploy `auto-generate-post --no-verify-jwt` → verify bài auto sáng 13/06.

**(session 29B — 2026-06-11)** — **Auto-post kèm ẢNH** (bài chữ thuần đơn điệu): (1) migration 009 bảng `page_photos` (kho ảnh + xoay vòng times_used/last_used_at); (2) edge function MỚI `harvest-page-photos` (cron 08:00 VN hằng ngày, kéo ảnh thật từ Graph `/{page_id}/photos?type=uploaded` về kho, refresh URL fbcdn vì có hạn); (3) `auto-generate-post`: TOPIC_POOL thêm kind product/tip, chọn ảnh ưu tiên: ảnh THẬT từ kho → bài "tip" không có ảnh thật thì Pollinations AI (pre-warm 30s, fail thì chữ thuần) → bài "product" không ảnh thật = chữ thuần (KHÔNG ảnh giả). `fb-post` đã hỗ trợ sẵn image_url→/photos, không sửa. ⏳ Ben: chạy migration 009 + deploy 2 functions + tạo cron harvest. Thử nghiệm 1-2 ngày (12-13/06).
