...
**Last updated:** 2026-06-12 (session 30, phần 2) — **Dọn landing + login cho trung thực & mới mẻ:** (1) Instagram thêm badge "Sắp có" (trước liệt kê như đang hỗ trợ — sai); (2) "Zalo 24/7" → "giờ hành chính" (khớp footer); (3) copy "7 chủ đề xoay vòng" lỗi thời → "ngẫu nhiên, học giọng shop"; (4) **XOÁ 3 testimonial bịa** (Phở Xương Bò, Nail & Spa ĐN, Shop Áo Nam CT) + 2 testimonial bịa ở login (Mỹ phẩm Hana, Cafe Mộc) — chỉ giữ Kho Nệm thật, thay bằng card mời "Shop của bạn?" (lý do: Meta reviewer vào site + nhất quán với việc cấm AI bịa chuyện khách); login gỡ dots + tắt auto-rotate; (5) **`demo-caption` nâng Haiku → `claude-sonnet-4-6`** (demo là bộ mặt, 3 lần/session nên rẻ). ⚠️ Cần Ben xác nhận tên "Chị Thu Hà" trên testimonial Kho Nệm có đúng người thật không. ⏳ Ben: commit + push (Render tự deploy HTML) + `npx.cmd supabase functions deploy demo-caption --no-verify-jwt`.

**(session 30 — phần 1)** — **Verify bài auto 12/06: giọng KHÁ HƠN nhưng còn 2 nhóm lỗi.** (1) Vẫn bịa từ ("chợp cơ hội", "để nặng đổ lên", câu gãy) → gốc rễ là Haiku yếu tiếng Việt, Ben chốt **nâng model auto-generate-post lên `claude-sonnet-4-6`** (~200đ/bài, 1 bài/ngày). (2) **Bịa flash sale + bịa chuyện khách** → thủ phạm là TOPIC_POOL có sẵn topic "flash sale hoặc ưu đãi sắp tới" + "câu chuyện khách hàng thật" → ĐÃ BỎ 2 topic này; thêm rule prompt: cấm bịa sự kiện/chuyện khách + xưng hô nhất quán (bài 12/06 nhảy "mình"→"E"). Lưu ý: Edit tool truncate cả index.ts LẪN PROGRESS.md → đều khôi phục từ HEAD + sửa bằng Python. `generate-caption` VẪN dùng Haiku (user-facing, nhiều call — chưa nâng). ✅ Ben đã commit (`9d5ad38`) + push + deploy. ⏳ Verify bài auto sáng 13/06 (calendar trước 10:00).

**(session 29B — 2026-06-11)** — **Auto-post kèm ẢNH** (bài chữ thuần đơn điệu): (1) migration 009 bảng `page_photos` (kho ảnh + xoay vòng times_used/last_used_at); (2) edge function MỚI `harvest-page-photos` (cron 08:00 VN hằng ngày, kéo ảnh thật từ Graph `/{page_id}/photos?type=uploaded` về kho, refresh URL fbcdn vì có hạn); (3) `auto-generate-post`: TOPIC_POOL thêm kind product/tip, chọn ảnh ưu tiên: ảnh THẬT từ kho → bài "tip" không có ảnh thật thì Pollinations AI (pre-warm 30s, fail thì chữ thuần) → bài "product" không ảnh thật = chữ thuần (KHÔNG ảnh giả). `fb-post` đã hỗ trợ sẵn image_url→/photos, không sửa. ✅ Ben đã chạy migration 009 + deploy + cron. **Verify 12/06: bài auto CÓ ẢNH THẬT từ kho (harvest hoạt động).** Nhánh Pollinations AI chưa từng kích hoạt (kho còn ảnh thật → đúng thiết kế, ảnh thật luôn ưu tiên). Phân biệt nguồn ảnh: URL `fbcdn.net` = thật, `pollinations.ai` = AI.

**(session 29 — 2026-06-11):** — **Verify bài auto 08:30 11/06: ĐÃ RA nhưng có lỗi từ ngữ AI bịa** ("quần ga" thay vì "chăn ga", "chập xuống" thay vì "xẹp lún", câu đứt gãy "Cơn mưa hay thay quần ga"). Fix: thêm block "CHÍNH TẢ & TỪ NGỮ" vào prompt cả `auto-generate-post` lẫn `generate-caption` (cấm bịa từ + nêu lỗi thực tế làm ví dụ + bắt tự rà soát từng câu trước khi trả về). ⚠️ Commit `cd9264c` trong repo là bản LỖI (Edit tool truncate file — bug đã biết); bản đúng đang STAGED chờ amend. **Ben cần chạy từ Windows PowerShell:** xoá `.git\HEAD.lock` + `.git\index.lock` → `git commit --amend` → push → deploy 2 functions (`npx.cmd supabase functions deploy auto-generate-post --no-verify-jwt` + `npx.cmd supabase functions deploy generate-caption`). ⏳ Verify bài auto sáng 12/06.
