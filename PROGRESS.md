...
**Last updated:** 2026-06-11 (session 29B) — **Auto-post kèm ẢNH** (bài chữ thuần đơn điệu): (1) migration 009 bảng `page_photos` (kho ảnh + xoay vòng times_used/last_used_at); (2) edge function MỚI `harvest-page-photos` (cron 08:00 VN hằng ngày, kéo ảnh thật từ Graph `/{page_id}/photos?type=uploaded` về kho, refresh URL fbcdn vì có hạn); (3) `auto-generate-post`: TOPIC_POOL thêm kind product/tip, chọn ảnh ưu tiên: ảnh THẬT từ kho → bài "tip" không có ảnh thật thì Pollinations AI (pre-warm 30s, fail thì chữ thuần) → bài "product" không ảnh thật = chữ thuần (KHÔNG ảnh giả). `fb-post` đã hỗ trợ sẵn image_url→/photos, không sửa. ⏳ Ben: chạy migration 009 + deploy 2 functions + tạo cron harvest. Thử nghiệm 1-2 ngày (12-13/06).
